type Color = string;
type Font = string;
type Size = string;

export const primary1: Color = '#2AC1BC';
export const greySpan: Color = '#717171';
export const greyBg1: Color = '#f6f6f6';
export const greyLine: Color = '#CCD3D3';
export const greyButton: Color = '#b5b5b5';
export const baemin: Color = '#4bc2cb';
export const green: Color = '#0ced39';
export const red1: Color = '#f56342';
export const red2: Color = '#ff6350';
export const lightBlack: Color = '#333333';
export const white: Color = '#ffffff';

export const baeminFont: Font = 'BMHANNAAir, sans-serif';
export const baeminThickFont: Font = 'BMDOHYEON, sans-serif';

export const normalContainerWidth: Size = '1200px';

export const normalRadius: Size = '5px';
